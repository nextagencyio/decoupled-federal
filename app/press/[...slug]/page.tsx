...
import { GET_PRESS_RELEASE_BY_PATH } from '@/lib/queries'
...
interface PressReleaseByPathData {
  route: {
    entity: DrupalPressRelease
  } | null
}
...
async function getPressRelease(path: string): Promise<DrupalPressRelease | null> {
  try {
    ...
    const { data } = await apolloClient.query<PressReleaseByPathData>({
      query: GET_PRESS_RELEASE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getPressRelease(path)

  if (!item) {
    return { title: 'Press Release Not Found | Federal' }
  }

  return {
    title: `${item.title} | Federal`,
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).contactName && (
                    <div>
                      <dt className="text-sm text-gray-500">Media Contact</dt>
